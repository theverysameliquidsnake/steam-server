...
func GetSteamAppDetails(appId uint32) error {
	...
	if !jsoniter.Get(body, strconv.FormatUint(uint64(appId), 10), "success").ToBool() {
		revertErr := repositories.SetStubNeedsUpdateAndSkipStatuses(appId, true, true)
		return errors.Join(errors.New("jsoniter: could not confirm success"), revertErr)
	}
	...
	if publicAppDetailsSteam.Type != "game" {
		...
		return errors.Join(errors.New("assertion: not a game type app"), revertErr)
	}
